...
const TEMPLATES: Record<Theme, Template> = {
  princess: {
    ...
    pages: (name) => [
      ...
      { text: `The squirrel led them to a cave full of glowing crystals.`, scene: 'a smiling young princess with a crown standing in the center, surrounded by several tall cartoon crystal shapes with sparkles, simple cave arch in the background, children coloring book style' },
  ...
  dinosaur: {
    ...
    pages: (name) => [
      ...
      { text: `Spike pointed to a big cave on the other side.`, scene: 'a small baby dinosaur with one paw raised pointing toward a round cave entrance with hanging vines, palm trees around' },
  ...
  space: {
    ...
    pages: (name) => [
      { text: `${name} loved looking up at the stars every night.`, scene: 'a smiling child leaning on a round window sill, a big crescent moon and a few scattered stars floating outside the window' },
  ...
  underwater: {
    ...
    pages: (name) => [
      ...
      { text: `Suddenly — a big shape swam by! A shark!`, scene: 'a child and a small mermaid under water pointing with surprised faces at a large cartoon shark swimming by with a friendly smile' },
      { text: `It was just a gentle giant basking shark, who waved a fin.`, scene: 'a friendly basking shark with kind eyes waving a fin' },
      { text: `Pearl led ${name} to a deep secret cave.`, scene: 'a child and a small mermaid swimming toward a round underwater cave entrance decorated with coral and seaweed, bubbles around' },
